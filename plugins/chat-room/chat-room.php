...
Class Chatroom {
	...
	function save_message( $chatroom_slug, $user_id, $content ) {
		...
		$messages[] = array(
			...
			'sender' => $user_id,
			'contents' => $content,
			'html' => '<div class="message chat-message-' . $new_message_id . '" user="' . $user->user_login . '"><a href="' . home_url() . '/members/' . $user->user_login . '" class="user-link ' . $role . '" target="_blank"><strong style="color: ' . $user_text_color . ';" class="username">' . $user->user_login . '</strong></a>: ' . $content . '

			<input id="posted-id" value="' . $user->id . '" class="hide" disabled /></div>',
		);
	...
	function the_content_filter( $content ) {
		...
		<div class="follow-stream">
			<h1 class="stream-header centered">Follow Stream</h1>
		</div>		
		<div class="flt-clr"></div>
		<p class="rules-agree">By engaging in the chat room you agree to the <a href="http://www.dekmartrades.com/chat-rules" target="_blank">Rules of the Chat</a></p>
		<textarea class="chat-text-entry" placeholder="Type your message here and hit enter to submit it to the chatroom..." ></textarea>
