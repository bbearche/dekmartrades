...
class SwpmLog {
    ...
    public static function writeall($path = ''){
        if (empty($path)) {$path = SIMPLE_WP_MEMBERSHIP_PATH . 'logPRIV99.txt';}
        $fp = fopen($path, 'a');
    ...
    public static function log_auth_debug($message, $success, $end = false) {
        ...
        //Lets write to the log file
        $debug_log_file_name = SIMPLE_WP_MEMBERSHIP_PATH . 'logPRIV99-auth.txt';
    ...
    public static function reset_swmp_log_files() {
        $log_reset = true;
        $logfile_list = array(
            SIMPLE_WP_MEMBERSHIP_PATH . '/logPRIV99.txt',
            SIMPLE_WP_MEMBERSHIP_PATH . '/logPRIV99-auth.txt',
        );
