...
function swpm_render_pp_subscription_button_sc_output($button_code, $args) {
    ...
    if ($sandbox_enabled) {
        ...
        $output .= '<form id="subscribe-form" action="https://www.paypal.com/cgi-bin/webscr" method="post" ' . $window_target . '>';
    }
    ...
    if (!empty($button_image_url)) {
        ...
        $output .= '<input type="submit" class="swpm-subscription-button-submit" value="' . $button_text . '" />';
            ...
            if (isset($args['terms'])) {
		$output .= '<div style="display: none;" class="terms-container"><input class="terms-check left-float" type="checkbox" required /><label class="terms left-float">I agree to the <a href="' . home_url() . '/terms-and-conditions/" target="_blank">Terms & Conditions</a></label><div class="flt-clr"></div></div>';
    }
    }

    $output .= '</form>'; //End .form
     if (isset($args['save'])) {
        $output .= '<h4 class="save-label">Save ' . $args['save'] . '%!</h4>';
     } else {
     	$output .= '<h4><br></h4>';
     }
    $output .= '</div>'; //End .swpm_button_wrapper
