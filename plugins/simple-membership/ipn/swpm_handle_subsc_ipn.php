...
function swpm_handle_subsc_signup_stand_alone($ipn_data,$subsc_ref,$unique_ref,$swpm_id='')
{
	...
	// Upgrade before table update to cancel original paypal profile
        do_action('swpm_upgrade_ipn', $ipn_data);
	...
	$email = $ipn_data['payer_email'];
        ...
        swpm_debug_log_subsc("Creating new member account. Unique ref: " . $unique_ref . " Membership level ID: ".$membership_level,true);
	...
	swpm_debug_log_subsc(json_encode($data),false);
	if (swpm_check_ip_account_allowed($user_ip)) {
		...
		swpm_debug_log_subsc("Member signup/upgrade completion email successfully sent to: ".$email,true);
		    ...
		    $subject = "Your dekmartrades membership";
		    $body = "Sorry, you already have an account.";
		    $from_address = $settings->get_value('email-from');
		    $headers = 'From: '.$from_address . "\r\n";
		    wp_mail($email,$subject,$body,$headers);
		    swpm_debug_log_subsc("Member signup/upgrade failure email sent to: ".$email,true);
...
function swpm_debug_log_subsc($message,$success,$end=false) 
{
    ...
    $debug_log_file_name = SIMPLE_WP_MEMBERSHIP_PATH . 'logPRIV99.txt';
...
function swpm_check_ip_account_allowed($ip_address) {
	    global $wpdb;
	    $settings = SwpmSettings::get_instance();
	    $members_table_name = $wpdb->prefix . "swpm_members_tbl";
	    $membership_level_table = $wpdb->prefix . "swpm_membership_tbl";
	    $membership_level = $subsc_ref;
    	swpm_debug_log_subsc("swpm_handle_subsc_signup_stand_alone(). IP Address: ".$ip_address,true);
        $query_db = $wpdb->get_row($wpdb->prepare("SELECT * FROM $members_table_name WHERE last_accessed_from_ip = %s", $ip_address), OBJECT);
	if ($query_db) {
		return false;
	} else {
		return true;
